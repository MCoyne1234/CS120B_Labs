...
int main(void)
{
    ...
    while (1) 
    {
        ...
        if(!button){
            if((button & 16) == 16) output += "R";
            if((button & 32) == 32) output += "L";
            if((button & 64) == 64) output += "X";
            if((button & 128) == 128) output += "A";
            if((button & 256) == 256) output += "Right";
            if((button & 512) == 512) output += "Left";
            if((button & 1024) == 1024) output += "Up";
            if((button & 2048) == 2048) output += "Down";
            if((button & 4096) == 4096) output += "Start";
            if((button & 8192) == 8192) output += "Select";
            if((button & 16384) == 16384) output += "Y";
            if((button & 32768) == 32768) output += "B";
        }else output = "None";            
        PORTD = output;
    }
}

unsigned char TestJunk(unsigned short button){
